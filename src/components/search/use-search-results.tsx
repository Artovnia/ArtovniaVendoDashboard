...
import {
  Shortcut,
  ShortcutType,
} from '../../providers/keybind-provider';
...
export const useSearchResults = ({
  ...
  const { dynamicResults, isFetching } =
    useDynamicSearchResults(area, limit, q);
...
const useDynamicSearchResults = (
  ...
  const isAreaFetching = useCallback(
    (area: SearchArea): boolean => {
      ...
      return (
        isAreaEnabled(currentArea, area) &&
        responseMap[area as keyof typeof responseMap]
          ?.isFetching
      );
    },
...
const transformMap: TransformMap = {
  ...
  taxRegion: {
    ...
    transform: (taxRegion: HttpTypes.AdminTaxRegion) => ({
      ...
      subtitle: taxRegion.province_code
  ...
  publishableApiKey: {
    dataKey: 'api_keys',
    transform: (
      apiKey: HttpTypes.AdminApiKeyResponse['api_key']
    ) => ({
      id: apiKey.id,
      title: apiKey.title,
