...
export const getPaymentsFromOrder = (
  order: HttpTypes.AdminOrder
) => {
  return order.payment_collections
    ?.map(
      (collection: HttpTypes.AdminPaymentCollection) =>
        collection.payments
    )
    .flat(1)
...
const Payment = ({
  ...
  const getPaymentStatusAttributes = (
  ...
  const [status, color] = getPaymentStatusAttributes(
    payment
  ) as [string, 'green' | 'orange' | 'red'];
