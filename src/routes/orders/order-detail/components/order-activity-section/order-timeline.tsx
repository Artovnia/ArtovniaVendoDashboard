...
const useActivityItems = (
  order: AdminOrder
): Activity[] => {
  const { t } = useTranslation();

  const { order_changes: orderChanges = [] } =
    useOrderChanges(order.id, {
      change_type: [
        'edit',
        'claim',
        'exchange',
  ...
  const missingLineItemIds = getMissingLineItemIds(
    order,
    rmaChanges
  );
  const { order_items: removedLineItems = [] } =
    useOrderLineItems(
      order.id,
  ...
  return useMemo(() => {
    ...
    for (const payment of payments) {
      ...
      if (payment.canceled_at) {
        items.push({
          title: t(
            'orders.activity.events.payment.canceled'
          ),
      ...
      if (payment.captured_at) {
        items.push({
          title: t(
            'orders.activity.events.payment.captured'
          ),
    ...
    for (const fulfillment of order.fulfillments || []) {
      items.push({
        title: t(
          'orders.activity.events.fulfillment.created'
        ),
      ...
      if (fulfillment.delivered_at) {
        items.push({
          title: t(
            'orders.activity.events.fulfillment.delivered'
          ),
      ...
      if (fulfillment.shipped_at) {
        items.push({
          title: t(
            'orders.activity.events.fulfillment.shipped'
          ),
      ...
      if (fulfillment.canceled_at) {
        items.push({
          title: t(
            'orders.activity.events.fulfillment.canceled'
          ),
    ...
    for (const ret of returns) {
      ...
      if (ret.canceled_at) {
        items.push({
          title: t(
            'orders.activity.events.return.canceled',
            {
              returnId: ret.id.slice(-7),
            }
          ),
      ...
      if (
        ...
        items.push({
          title: t(
            'orders.activity.events.return.received',
            {
              returnId: ret.id.slice(-7),
            }
          ),
          ...
          itemsMap,
          children: (
            <ReturnBody orderReturn={ret} isReceived />
          ),
        });
      }
    }
    ...
    for (const exchange of exchanges) {
      const exchangeReturn = returnMap.get(
        exchange.return_id!
      );
      ...
      items.push({
        ...
        timestamp:
          exchange.canceled_at || exchange.created_at,
    ...
    for (const edit of orderChanges.filter(
      ...
      items.push({
        title: t(
          `orders.activity.events.edit.${edit.status}`,
          {
    ...
    for (const transfer of orderChanges.filter(
      ...
      if (transfer.requested_at) {
        items.push({
          title: t(
            `orders.activity.events.transfer.requested`,
            {
          ...
          children: (
            <TransferOrderRequestBody transfer={transfer} />
          ),
        });
      }
      ...
      if (transfer.confirmed_at) {
        items.push({
          title: t(
            `orders.activity.events.transfer.confirmed`,
            {
              transferId: transfer.id.slice(-7),
            }
          ),
      ...
      if (transfer.declined_at) {
        items.push({
          title: t(
            `orders.activity.events.transfer.declined`,
            {
              transferId: transfer.id.slice(-7),
            }
          ),
        ...
        items.push({
          title: (
            <ChangeDetailsTooltip
              title={t(
                `orders.activity.events.update_order.shipping_address`
              )}
      ...
      if (updateType === 'billing_address') {
        items.push({
          title: (
            <ChangeDetailsTooltip
              title={t(
                `orders.activity.events.update_order.billing_address`
              )}
              previous={getFormattedAddress({
                address: update.actions[0].details.old,
              }).join(', ')}
      ...
      if (updateType === 'email') {
        items.push({
          title: (
            <ChangeDetailsTooltip
              title={t(
                `orders.activity.events.update_order.email`
              )}
  ...
  return (
    <div className='grid grid-cols-[20px_1fr] items-start gap-2'>
      <div className='flex size-full flex-col items-center gap-y-0.5'>
        ...
        {!isFirst && (
          <div className='bg-ui-border-base w-px flex-1' />
        )}
      </div>
      <div
        className={clx({
...
const ReturnBody = ({
  ...
  const { mutateAsync: cancelReturnRequest } =
    useCancelReturn(orderReturn.id, orderReturn.order_id);
  ...
  const numberOfItems = orderReturn.items.reduce(
    (acc, item) => {
      return (
        acc +
        (isReceived
          ...
          : item.quantity)
      ); // TODO: revisit when we add dismissed quantity on ReturnItem
    },
    0
  );
  ...
  return (
    <div className='flex items-start gap-1'>
      ...
      {isCreated && (
        <>
          <div className='mt-[2px] flex items-center leading-none'>
            ⋅
          </div>
          <Button
            onClick={onCancel}
  ...
  return (
    <div>
      {outboundItems > 0 && (
        <Text size='small' className='text-ui-fg-subtle'>
          {t(
            'orders.activity.events.exchange.itemsInbound',
            {
              count: outboundItems,
            }
          )}
      ...
      {inboundItems > 0 && (
        <Text size='small' className='text-ui-fg-subtle'>
          {t(
            'orders.activity.events.exchange.itemsOutbound',
            {
              count: inboundItems,
            }
          )}
...
const TransferOrderRequestBody = ({
  ...
  const { mutateAsync: cancelTransfer } =
    useCancelOrderTransfer(transfer.order_id);
...
function countItemsChange(
  ...
  actions.forEach((action) => {
    ...
    if (action.action === 'ITEM_UPDATE') {
      const quantityDiff = action.details!
        .quantity_diff as number;
 ...
 */
function getMissingLineItemIds(
  order: AdminOrder,
  changes: AdminOrderChange[]
) {
  if (!changes?.length) {
    return [];
  }
  ...
  const existingItemsMap = new Map(
    order.items.map((item) => [item.id, true])
  );
  ...
  changes.forEach((change) => {
    change.actions.forEach((action) => {
      ...
      if (
        (action.details.reference_id as string).startsWith(
          'ordli_'
        ) &&
