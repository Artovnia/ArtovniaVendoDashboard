...
export const EditProductMediaForm = ({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      const oldIndex = fields.findIndex(
        (item) => item.field_id === active.id
      );
      const newIndex = fields.findIndex(
        (item) => item.field_id === over?.id
      );
  ...
  const handleSubmit = form.handleSubmit(
    async ({ media }) => {
      ...
      const withUpdatedUrls = media.map((entry, i) => {
        const toUploadIndex = filesToUpload.findIndex(
          (m) => m.index === i
        );
        if (toUploadIndex > -1) {
          return {
  ...
  const handlePromoteToThumbnail = () => {
    ...
    const currentThumbnailIndex = fields.findIndex(
      (m) => m.isThumbnail
    );
