...
export const CreateProductVariantForm = ({
  ...
  const [tab, setTab] = useState<
    Tab.DETAIL | Tab.PRICE | Tab.INVENTORY
  >(Tab.DETAIL);
  const [tabState, setTabState] =
    useState<TabState>(initialTabState);
  ...
  useEffect(() => {
    if (
      ...
      inventoryField.fields.length === 0
    ) {
  ...
  const handleChangeTab = (
    update: Tab.DETAIL | Tab.PRICE | Tab.INVENTORY
  ) => {
    if (tab === update) {
      return;
    }
    ...
    const tabs = tabOrder.slice(
      0,
      tabOrder.indexOf(update)
    );
  ...
  const handleNextTab = (
    tab: Tab.DETAIL | Tab.PRICE | Tab.INVENTORY
  ) => {
